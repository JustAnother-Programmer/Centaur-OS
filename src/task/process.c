...
static struct process* processes[CENTAUROS_MAX_PROCESSES] = {};
...
{
    if(process_id < 0 || process_id >= CENTAUROS_MAX_PROCESSES)
    {
        return NULL;
    }
...
{
    for(int i = 0; i < CENTAUROS_MAX_PROCESSES; i++)
    {
        if(processes[i] == 0)
            return i;
    }
...
int process_load_for_slot(const char* filename, struct process** process, int process_slot)
{
    ...
    program_stack_ptr = kzalloc(CENTAUROS_USER_RPOGRAM_STACK_SIZE);
