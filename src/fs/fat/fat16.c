...
#define CENTAUROS_FAT16_SIGNATURE 0x29
#define CENTAUROS_FAT16_FAT_ENTRY_SIZE 0x02
#define CENTAUROS_FAT16_BAD_SECTOR 0xFF7
#define CENTAUROS_FAT16_UNUSED 0x00
...
int fat16_get_total_items_for_directory(struct disk* disk, uint32_t directory_start_sector)
{
    ...
    if(diskstreamer_seek(stream, directory_start_pos) != CENTAUROS_ALL_OK)
    {
        res = -EIO;
        goto out;
    }
    ...
    while(1)
    {
        if(diskstreamer_read(stream, &item, sizeof(item)) != CENTAUROS_ALL_OK)
        {
            res = -EIO;
            goto out;
        }
...
int fat16_get_root_directory(struct disk* disk, struct fat_private* fat_private, struct fat_directory* directory)
{
    ...
    if(diskstreamer_seek(stream, fat16_sector_to_absolute(disk, root_dir_sector_pos)) != CENTAUROS_ALL_OK)
    {
        res = -EIO;
        goto out;
    }

    if(diskstreamer_read(stream, dir, root_dir_size) != CENTAUROS_ALL_OK)
    {
        res = -EIO;
        goto out;
    }
...
int fat16_resolve(struct disk* disk)
{
    ...
    if(diskstreamer_read(stream, &fat_private->header, sizeof(fat_private->header)) != CENTAUROS_ALL_OK)
    {
        res = -EIO;
        goto out;
    }
    ...
    if(fat16_get_root_directory(disk, fat_private, &fat_private->root_directory) != CENTAUROS_ALL_OK)
    {
        res = -EIO;
        goto out;
    }
...
static int fat16_get_fat_entry(struct disk* disk, int cluster)
{
    ...
    res = diskstreamer_seek(stream, fat_table_position * (cluster * CENTAUROS_FAT16_FAT_ENTRY_SIZE));
...
static int fat16_get_cluster_for_offset(struct disk* disk, int starting_cluster, int offset)
{
    ...
    for(int i = 0; i < clusters_ahead; i++)
    {
        ...
        if(entry == CENTAUROS_FAT16_BAD_SECTOR)
        {
            res = -EIO;
            goto out;
        }
...
struct fat_item* fat16_find_item_in_directory(struct disk* disk, struct fat_directory* directory, const char* name)
{
    ...
    char tmp_filename[CENTAUROS_MAX_PATH];
